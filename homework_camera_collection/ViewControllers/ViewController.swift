...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        mapView.register(ImageInfoAnnotationView.self, forAnnotationViewWithReuseIdentifier: "view")
    }

    //MARK: - Functions
    @IBAction func addButtonPressed(_ sender: Any) {
        let imageManager = ImageManager(viewController: self)
        ...
        alertController.addAction(
            .init(title: "Gallery", style: .default, handler: { _ in
                imageManager.requestImageFromGallery()
            }))
        alertController.addAction(
            .init(title: "Camera", style: .default, handler: { _ in
                imageManager.requestImageFromCamera()
            })
        )
        alertController.addAction(
...
extension ViewController: ImageManagerDelegate {
    func imageReceived(image: UIImage) {
        let imageInfo = ImageInfoAnnotation(
            image: image,
            date: Date(),
...
extension ViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as! ImageInfoCollectionViewCell
        cell.configure(info: data[indexPath.row])
        return cell
    }
}
